...
export default function DealerDashboard() {
  ...
  // inline edit state
  const [editing, setEditing] = React.useState(null); // vin being edited
  ...
  function parseCsv(file) {
    ...
    Papa.parse(file, {
      ...
      skipEmptyLines: true,
      complete: (res) => {
        // Normalize headers -> lowercased keys we expect
  ...
  return (
    <section style={{maxWidth:1100, margin:"24px auto"}}>
      ...
      <h3 style={{marginTop:24}}>Add / Update Single Vehicle</h3>
      ...
      <p style={{opacity:.8, marginTop:-6}}>Headers can be <code>vin,year,make,model,trim,price,mileage,location</code> (case-insensitive).</p>
      ...
      {csvPreview.length > 0 && (
        <div style={{marginTop:10, maxHeight:200, overflow:"auto", border:"1px solid #eee"}}>
          <table style={{width:"100%", borderCollapse:"collapse", fontSize:13}}>
            <thead><tr>
              <th>VIN</th><th>Year</th><th>Make</th><th>Model</th><th>Trim</th><th>Price</th><th>Mileage</th><th>Location</th>
            </tr></thead>
            <tbody>
